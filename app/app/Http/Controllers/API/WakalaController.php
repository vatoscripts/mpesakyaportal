...
class WakalaController extends GuzzleController
{
    public function __construct()
    {
        ...
        $this->middleware(['role:AGGREGATOR,WAKALA_RECRUITER,TDR'])->except(['viewUnverifiedWakala', 'verifyWakala', 'getApprovedWakala', 'getDeclinedWakala', 'getUnverifiedWakala', 'getNewUnverifiedWakala',  'getWakalaListWithStatus', 'viewNewUnverifiedWakala', 'verifyNewWakala', 'getNewApprovedWakala']);
        $this->middleware(['role:ADMIN,MPESA_ADMIN,BACKOFFICE'])->only(['viewUnverifiedWakala', 'verifyWakala', 'getApprovedWakala', 'getDeclinedWakala', 'getUnverifiedWakala', 'getWakalaListWithStatus', 'getNewUnverifiedWakala', 'viewNewUnverifiedWakala', 'verifyNewWakala', 'getNewApprovedWakala']);
    }
    ...
    public function getNewUnverifiedWakala(){
        
        $url = 'GetUnverifiedNewWakalas';
    ...
    public function viewNewUnverifiedWakala(Request $request)
    {
        $body = [
            'UserID' => $this->user['UserID'],
            'MpesaAgentID' => $request->agentID,
    ...
    public function verifyNewWakala(WakalaRequest $request)
    {
        ...
        Log::channel('New-Wakala-verify')->debug(['user' => $this->user['UserName'], 'Request' => $body, 'Response' => $data]);

        if ($data['ID'] == 0) {

            return response()->json(['message' => 'Successful new wakala verification !', 'Response' => $data], 200);
    ...
    public function getNewApprovedWakala()
    {
